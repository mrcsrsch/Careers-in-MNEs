##################################################################################
### Robustness check: Regressions with base salary only (no overtime & extra bonuses) #####
##################################################################################
#### packages ######
...
if (!dir.exists(paste0(map_output, "robustness/wage_basic/"))) dir.create(paste0(map_output, "robustness/wage_basic/"))

map_output_here <- paste0(map_output, "robustness/wage_basic/") # for final outputs
...
###### A) Mover wage growth #####
if (across.firm.interactions){
  pdf(file=paste0(map_output_here, "coefplot_across_firm_w_across_basic.pdf"), width=2.10*5.5, height=2.97*5.5)
  ...
  pdf(file=paste0(map_output_here, "coefplot_across_firm_wo_across_basic.pdf"), width=16*0.7, height=9*0.7)
...
if (across.firm.interactions){
  pdf(file=paste0(map_output_here, "coefplot_stayer_wages_w_across_basic.pdf"), width=4*3.2, height=3*3.2)
  ...
  pdf(file=paste0(map_output_here, "coefplot_stayer_wages_wo_across_basic.pdf"), width=16*0.7, height=9*0.7)
