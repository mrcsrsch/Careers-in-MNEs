##################################################################################
### Robustness check: MNE vs. industry experience #####
##################################################################################
#### packages ######
...
if (!dir.exists(paste0(map_output, "robustness/industry_exp/"))) dir.create(paste0(map_output, "robustness/industry_exp/"))

map_output_here <- paste0(map_output, "robustness/industry_exp/") # for final outputs
...
##################################################################################
##################################################################################
#### Identify industry share of MNEs by 2-digit industry-year (employment weighted) #### 
POLIS <- merge(POLIS, OGs[, c("OG_BEID", "year", "company.type.1", "NACE_LETTER", "NACE_2digit")], by=c("OG_BEID", "year"), all.x=T)
rm(OGs)
industry_shares <- POLIS[, sapply(c("MNE", "INT_COMP"), function(x) sum(company.type.1==x)/.N, simplify = F, USE.NAMES = T), by=c("NACE_2digit", "year")]
setnames(industry_shares, c("MNE", "INT_COMP"), c("MNE_share", "INT_COMP_share"))
rm(POLIS)
# add to reg.table
reg.table <- merge(reg.table, industry_shares, by=c("NACE_2digit", "year"))

##### Calculate weighted average of past-MNE share of worker's industries ###### 
setorderv(reg.table, c("worker.ID", "year", "Nemployer.year"))
reg.table[, MNE_share_avg := cumsum(MNE_share)/seq_along(MNE_share)*100, by=worker.ID]

#### calculate previous years of experience in same industry as current one (2 digit industry code, e.g. Manufacturing of food products) ######
setorderv(reg.table, c("worker.ID", "year", "Nemployer.year"))
reg.table[, industry_exp := cumsum(tenure_years_OGBEID_spell), by=c("worker.ID", "NACE_2digit")]
## cut industry experience into splines 
reg.table[, industry_exp_group := cut(industry_exp, breaks=c(-Inf, seq(1,10,1), Inf), include.lowest=TRUE)]
...
reg_main <- feols(fml=as.formula(paste0("lhwage_detrended ~ ",
                                      ...
                                      across_firm_interact,
                                     controls,
                                     " + i(industry_exp_group, ref=\"[-Inf,1]\")",
                                     "+ MNE_share_avg",
                                        "",
                                        "| ",
                                        # fixed effects
...
etable(reg_main, #lapply(paste0("regs[[",length(regs):1, "]]"), function(x) eval(parse(text=x))), 
       file=paste0(map_output_here, "reg_wages_non_linear_", ifelse(across.firm.interactions, "w_", "wo_"), "across", "_industry.tex"),  
       title="Wage profiles",
       label="tab:main_wage", 
       depvar = TRUE,
       digits = "r4",
...
###### A) Mover wage growth #####
if (across.firm.interactions){
  pdf(file=paste0(map_output_here, "coefplot_across_firm_w_across_industry.pdf"), width=2.10*5.5, height=2.97*5.5)
  ...
  for (type in types){
    ...
    # Also output estimates to a _industry.txt file
    sink(paste0(map_output_here, "coefs_across_firm_w_across_", type, "_total_industry.txt"))
    ...
    # Also output estimates to a _industry.txt file
    sink(paste0(map_output_here, "coefs_across_firm_w_across_", type, "_excess_industry.txt"))
    estimates.table(estimates, df = degrees_freedom(reg_main, type="t"))
    sink()
  }
  ...
  pdf(file=paste0(map_output_here, "coefplot_across_firm_wo_across_bw_industry.pdf"), width=16*0.7, height=9*0.7)
  ...
  pdf(file=paste0(map_output_here, "coefplot_across_firm_wo_across_col_industry.pdf"), width=16*0.7, height=9*0.7)
  estimates <- total.general.prem(reg_main, exp.types = c("MNE", "INT_COMP", "DOMESTIC"), k=1)
  company.type.plot(estimates, type="Total", legend.add = TRUE, 
                    legend.names = c("MNE", "Int. firm"), 
  ...
  # Also output estimates to a _industry.txt file
  sink(paste0(map_output_here, "coefs_across_firm_wo_across_industry.txt"))
  estimates.table(estimates, df = degrees_freedom(reg_main, type="t"))
  sink()
...
if (across.firm.interactions){
  pdf(file=paste0(map_output_here, "coefplot_stayer_wages_w_across_industry.pdf"), width=4*3.2, height=3*3.2)
  ...
  # Also output estimates to a _industry.txt file
  sink(paste0(map_output_here, "coefs_stayer_wages_w_across_industry.txt"))
  estimates.table(estimates, df = degrees_freedom(reg_main, type="t"))
  sink()
  
} else{
  # black and white (paper)
  pdf(file=paste0(map_output_here, "coefplot_stayer_wages_wo_across_bw_industry.pdf"), width=16*0.7, height=9*0.7)
  ...
  # color (presentation)
  pdf(file=paste0(map_output_here, "coefplot_stayer_wages_wo_across_col_industry.pdf"), width=16*0.7, height=9*0.7)
  estimates <- stayer.wages.estimates(reg_main, company.types=c("MNE", "INT_COMP", "DOMESTIC"),  k=1)
  stayer.wages.plot(estimates, legend.add=TRUE, highlight.MNE=TRUE, title="", 
  ...
  # Also output estimates to a _industry.txt file
  sink(paste0(map_output_here, "coefs_stayer_wages_wo_across_industry.txt"))
  estimates.table(estimates, df = degrees_freedom(reg_main, type="t"))
  sink()
}
rm(estimates)