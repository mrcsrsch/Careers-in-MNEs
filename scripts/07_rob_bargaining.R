##################################################################################
### Robestness check: Bargaining controls (lagged wage and firm size) #####
...
if (!dir.exists(paste0(map_output, "robustness/bargaining/"))) dir.create(paste0(map_output, "robustness/bargaining/"))

map_output_here <- paste0(map_output, "robustness/bargaining/") # for final outputs
...
##### lag company size and wage ###### 
setorderv(reg.table, c("worker.ID", "year", "Nemployer.year"))
reg.table[, c("lhwage_detrended.lag", "company.group.size.lag") := shift(.(lhwage_detrended, company.group.size), n=1, type="lag", fill=NA), by="worker.ID"]

# calculate wage and employment growth
reg.table[, c("lhwage_detrended.growth", "company.group.size.growth") := .(lhwage_detrended-lhwage_detrended.lag, log(company.group.size)-log(company.group.size.lag))]
reg.table[is.na(lhwage_detrended.growth), lhwage_detrended.growth := 0]
reg.table[is.na(company.group.size.growth), company.group.size.growth := 0]
...
reg_main <- feols(fml=as.formula(paste0("lhwage_detrended ~ ",
                                        ...
                                        across_firm_interact,
                                        controls,
                                        "+ lhwage_detrended.lag + log(company.group.size.lag)",
                                        "",
                                        "| ",
                                        # fixed effects
                                        "worker.ID ",
...
etable(reg_main, #lapply(paste0("regs[[",length(regs):1, "]]"), function(x) eval(parse(text=x))), 
       file=paste0(map_output_here, "reg_wages_non_linear_", ifelse(across.firm.interactions, "w_", "wo_"), "across", "_bargain.tex"),  
       title="Wage profiles",
       label="tab:main_wage", 
       depvar = TRUE,
       digits = "r4",
...
###### A) Mover wage growth #####
if (across.firm.interactions){
  pdf(file=paste0(map_output_here, "coefplot_across_firm_w_across_bargain.pdf"), width=2.10*5.5, height=2.97*5.5)
  ...
  for (type in types){
    ...
    # Also output estimates to a _bargain.txt file
    sink(paste0(map_output_here, "coefs_across_firm_w_across_", type, "_total_bargain.txt"))
    ...
    # Also output estimates to a _bargain.txt file
    sink(paste0(map_output_here, "coefs_across_firm_w_across_", type, "_excess_bargain.txt"))
    estimates.table(estimates, df = degrees_freedom(reg_main, type="t"))
    sink()
  }
  ...
  pdf(file=paste0(map_output_here, "coefplot_across_firm_wo_across_bw_bargain.pdf"), width=16*0.7, height=9*0.7)
  ...
  pdf(file=paste0(map_output_here, "coefplot_across_firm_wo_across_col_bargain.pdf"), width=16*0.7, height=9*0.7)
  estimates <- total.general.prem(reg_main, exp.types = c("MNE", "INT_COMP", "DOMESTIC"), k=1)
  company.type.plot(estimates, type="Total", legend.add = TRUE, 
                    legend.names = c("MNE", "Int. firm"), 
  ...
  # Also output estimates to a _bargain.txt file
  sink(paste0(map_output_here, "coefs_across_firm_wo_across_bargain.txt"))
  estimates.table(estimates, df = degrees_freedom(reg_main, type="t"))
  sink()
...
if (across.firm.interactions){
  pdf(file=paste0(map_output_here, "coefplot_stayer_wages_w_across_bargain.pdf"), width=4*3.2, height=3*3.2)
  ...
  # Also output estimates to a _bargain.txt file
  sink(paste0(map_output_here, "coefs_stayer_wages_w_across_bargain.txt"))
  estimates.table(estimates, df = degrees_freedom(reg_main, type="t"))
  sink()
  
} else{
  # black and white (paper)
  pdf(file=paste0(map_output_here, "coefplot_stayer_wages_wo_across_bw_bargain.pdf"), width=16*0.7, height=9*0.7)
  ...
  # color (presentation)
  pdf(file=paste0(map_output_here, "coefplot_stayer_wages_wo_across_col_bargain.pdf"), width=16*0.7, height=9*0.7)
  estimates <- stayer.wages.estimates(reg_main, company.types=c("MNE", "INT_COMP", "DOMESTIC"),  k=1)
  stayer.wages.plot(estimates, legend.add=TRUE, highlight.MNE=TRUE, title="", 
  ...
  # Also output estimates to a _bargain.txt file
  sink(paste0(map_output_here, "coefs_stayer_wages_wo_across_bargain.txt"))
  estimates.table(estimates, df = degrees_freedom(reg_main, type="t"))
  sink()
}
rm(estimates)
